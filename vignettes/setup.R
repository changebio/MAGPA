...
if (requireNamespace("rmarkdown") && !rmarkdown::pandoc_available("1.14")) stop("These vignettes assume pandoc version 1.14; older versions may not work.")
...
indexfns <- function(fns, text = paste0("`", fns, "`"), show = TRUE, pkg = "rgl") {
    ...
    anchors <- paste0("<a name=\"", fns, "\">", if (show) 
        linkfn(fns, text, pkg = pkg), "</a>")
    paste(anchors, collapse = if (show) 
        ", " else "")
}

indexclass <- indexproperties <- function(fns, text = paste0("`", fns, "`"), show = TRUE) {
    documentedfns <<- c(documentedfns, fns)
    anchors <- paste0("<a name=\"", fns, "\">", if (show) 
        text, "</a>")
    paste(anchors, collapse = if (show) 
...
linkfn <- function(fn, text = paste0("`", fn, "`"), pkg = NA) {
    if (is.na(pkg)) 
        paste0("<a href=\"#", fn, "\">", text, "</a>") else paste0("<a href=\"../../", pkg, "/help/", fn, "\">", text, "</a>")
}
...
# This displays the string code as `r code` when entered as `r rinline(code)`.  Due to Stephane Laurent
rinline <- function(code, script = FALSE) {
    if (script) 
        html <- "`r CODE`" else html <- "<code  class=\"r\">``` `r CODE` ```</code>"
    sub("CODE", code, html)
}
